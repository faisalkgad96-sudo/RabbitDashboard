...
def parse_and_compute_all(df):
    ...
    for col in timestamp_cols:
        if col in df.columns:
            ...
            df[col] = pd.NaT
    df['Resolution Time'] = (df['Resolved At'] - df['Created At']).dt.total_seconds() / 60
    df['On Queue Time'] = (df['Assigned To At'] - df['Created At']).dt.total_seconds() / 60
    fp = df['First Pending At'].fillna(df['Resolved At'])
    df['Handling Time'] = (fp - df['Assigned To At']).dt.total_seconds() / 60
    df['Interval'] = df['Created At'].apply(lambda x: calculate_interval(x) if pd.notnull(x) else np.nan)
